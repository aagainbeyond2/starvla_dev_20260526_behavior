...
class TrainerUtils:
    ...
    def _reset_dataloader(dataloader, epoch_counter):
        ...
        # After accelerator.prepare(), the dataloader is wrapped (e.g. DataLoaderShard)
        # and `dataloader.sampler` is replaced by a default SequentialSampler that has no
        # `set_epoch`, so calling `dataloader.sampler.set_epoch` silently no-ops and the
        # epoch never advances (every epoch would then re-draw the *same* samples because
        # sampling is keyed by `safe_hash((epoch, index, seed))`). Prefer accelerate's own
        # `DataLoaderShard.set_epoch`, which correctly routes to the underlying custom
        # sampler / dataset, and only fall back to the raw sampler for unwrapped loaders.
        epoch_set = False
        if callable(getattr(dataloader, "set_epoch", None)):
            dataloader.set_epoch(epoch_counter)
            epoch_set = True
        if not epoch_set and hasattr(dataloader, "sampler") and callable(
