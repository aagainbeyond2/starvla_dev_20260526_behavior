...
class DatasetStatisticalValues(BaseModel):
    ...
    per_timestamp_mean: Optional[NDArray] = Field(
        default=None, description="Per-timestamp mean values for sequence actions"
    )
    per_timestamp_std: Optional[NDArray] = Field(
        default=None, description="Per-timestamp std values for sequence actions"
    )
    per_timestamp_q01: Optional[NDArray] = Field(
        default=None, description="Per-timestamp 1st percentile values for sequence actions"
    )
    per_timestamp_q99: Optional[NDArray] = Field(
        default=None, description="Per-timestamp 99th percentile values for sequence actions"
    )

    @field_serializer("*", when_used="json")
    def serialize_ndarray(self, v: NDArray) -> Any:
        return v.tolist() if v is not None else v  # type: ignore
