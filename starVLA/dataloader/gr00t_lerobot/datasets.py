...
def _normalize_action_mode(mode: str) -> str:
    """Normalize action mode names and reject unsupported values."""
    ...
    if mode in {"absolute", "raw"}:
        mode = "abs"
    valid_modes = {"abs", "delta", "rel", "b1k_delta"}
    if mode not in valid_modes:
        raise ValueError(f"Invalid action_mode: {mode}. Expected one of: {sorted(valid_modes)}.")
    return mode
...
class LeRobotMixtureDataset(Dataset):
    ...
    def compute_overall_statistics(
        ...
        for modality in modality_keys:
            ...
            # Collect min, max, q01, q99 from all tasks. Keep optional
            # per-timestamp stats so behavior-style q99 normalization survives
            # metadata merging in multi-dataset mixtures.
            min_list = []
            max_list = []
            q01_list = []
            q99_list = []
            per_timestamp_mean_list = []
            per_timestamp_std_list = []
            per_timestamp_q01_list = []
            per_timestamp_q99_list = []
            has_per_timestamp_stats = per_task_stats[0][modality].get("per_timestamp_mean") is not None
            ...
            for task_idx, task_stats in enumerate(per_task_stats):
                ...
                if has_per_timestamp_stats:
                    if stats.get("per_timestamp_mean") is None or stats.get("per_timestamp_std") is None:
                        has_per_timestamp_stats = False
                    else:
                        per_timestamp_mean_list.append(np.array(stats["per_timestamp_mean"]))
                        per_timestamp_std_list.append(np.array(stats["per_timestamp_std"]))
                        per_timestamp_q01_list.append(np.array(stats["per_timestamp_q01"]))
                        per_timestamp_q99_list.append(np.array(stats["per_timestamp_q99"]))
            ...
            if has_per_timestamp_stats:
                pt_means = np.stack(per_timestamp_mean_list, axis=0)
                pt_stds = np.stack(per_timestamp_std_list, axis=0)
                pt_q01 = np.stack(per_timestamp_q01_list, axis=0)
                pt_q99 = np.stack(per_timestamp_q99_list, axis=0)
                weighted_pt_mean = np.average(pt_means, axis=0, weights=normalized_weights)
                weighted_pt_square = np.average(pt_stds**2 + pt_means**2, axis=0, weights=normalized_weights)
                weighted_pt_std = np.sqrt(np.maximum(weighted_pt_square - weighted_pt_mean**2, 0.0))
                if percentile_mixing_method == "weighted_average":
                    weighted_pt_q01 = np.average(pt_q01, axis=0, weights=normalized_weights)
                    weighted_pt_q99 = np.average(pt_q99, axis=0, weights=normalized_weights)
                else:
                    weighted_pt_q01 = np.min(pt_q01, axis=0)
                    weighted_pt_q99 = np.max(pt_q99, axis=0)
                weighted_pt_mean = weighted_pt_mean.tolist()
                weighted_pt_std = weighted_pt_std.tolist()
                weighted_pt_q01 = weighted_pt_q01.tolist()
                weighted_pt_q99 = weighted_pt_q99.tolist()
            else:
                weighted_pt_mean = None
                weighted_pt_std = None
                weighted_pt_q01 = None
                weighted_pt_q99 = None
            ...
            overall_stats[modality] = {
                ...
                "q01": weighted_q01,
                "q99": weighted_q99,
                "per_timestamp_mean": weighted_pt_mean,
                "per_timestamp_std": weighted_pt_std,
                "per_timestamp_q01": weighted_pt_q01,
                "per_timestamp_q99": weighted_pt_q99,
            }
